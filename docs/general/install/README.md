...
```hcl
module "rds" {
    source = "github.com/pbs/terraform-rds-module?ref=x.y.z"
}
```
...
gh -R 'pbs/terraform-rds-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/rds'
rm -f release.tar.gz
```
...
```bash
git remote add -f rds git@github.com:pbs/terraform-rds-module.git
git subtree add --prefix terraform/modules/rds rds main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/rds rds $REF --squash
```
